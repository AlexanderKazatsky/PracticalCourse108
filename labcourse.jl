...
# ╔═╡ 5b029fea-37e4-497c-b54b-5cfbfd80c6c1
#r = [t, x, y, z, E, px, py, pz]
...
function euler(f, r0, par, tmax, dt)
    ...
    for t in ts
		...
		r = r .+ dt .* f(r, par) # As discussed in theory
    end
...
function predictor_corrector(f, r0, par, tmax, dt)
    ...
    for t in ts
        ...
        ### Calculate all parameters of the state array (Hint: use broadcasting)
		r = r .+ dt/2 .* (f(r,par) .+ f(r_pred,par)) # As discussed in theory
    end
...
function runge_kutta_4(f, r0, par, tmax, dt)
    ...
    for t in ts
       ...
       ### Calculate all parameters of the state array (Hint: use broadcasting, examplle of it is already in this function)
		r = r .+ (k1 .+ 2 .* k2.+ 2 .* k3 .+ k4) ./ 6 # As discussed in theory
    end
...
begin
	rs1 = euler(eom, r0, par, tmax, dt)
	rs2 = predictor_corrector(eom, r0, par, tmax, dt)
	rs3 = runge_kutta_4(eom, r0, par, tmax, dt)
	# To be able to show all three methods I renamed the arrays.
end
...
# ╔═╡ 54518290-c3c9-4361-8ada-695e8ba4e4c8
begin
	plot1 = plot([r[1] for r in rs1],[r[2] for r in rs1],xlabel="t", ylabel="x",title="x(t)",label="euler")
	plot!([r[1] for r in rs2],[r[2] for r in rs2],label="predictor_corrector")
	plot!([r[1] for r in rs3],[r[2] for r in rs3],label="runge_kutta_4")
	savefig("x(t).png")
	plot1
end

# ╔═╡ 8a7ebfc3-c22b-4a5a-9e59-9130166930e3
begin
	plot2 = plot([r[1] for r in rs1],[r[3] for r in rs1],xlabel="t", ylabel="y",title="y(t)",label="euler")
	plot!([r[1] for r in rs2],[r[3] for r in rs2],label="predictor_corrector")
	plot!([r[1] for r in rs3],[r[3] for r in rs3],label="runge_kutta_4")
	savefig("y(t).png")
	plot2
end

# ╔═╡ 309c0cde-e0b6-496c-9233-31e440f0ce9d
begin
	plot3 = plot([r[1] for r in rs1],[r[4] for r in rs1],xlabel="t", ylabel="z",title="z(t)",label="euler")
	plot!([r[1] for r in rs2],[r[4] for r in rs2],label="predictor_corrector")
	plot!([r[1] for r in rs3],[r[4] for r in rs3],label="runge_kutta_4")
	savefig("z(t).png")
	plot3
end
...
begin
	# While energy conservation can graphically be seen in the above plot, in the following the individual values are compared.
	En1 = [r[5] for r in rs1]
	En2 = [r[5] for r in rs2]
	En3 = [r[5] for r in rs3]
	
	print("Energy conservation: ", all(x -> x == En1[1], En1), all(x -> x == En2[1], En2), all(x -> x == En3[1], En3))
end

# ╔═╡ e8f47304-1fde-4051-bae0-a366d93d465b
# Eventhough all three methods are plotted, only green curves are visible, because the results overlap. Only in the x(t) and y(x) graphs a slight deviation of the euler method can be seen.
...
begin
	...
	# Because the CDF is the integral of the PDF, a discrete integration is performed.
	cdf_values = [bin_contents[1]] # The array starts with the first column of the distribution histogram.

	for n in bin_contents[2:end]
		push!(cdf_values, cdf_values[end] + n) # For every following addition, the distribution value is added to the currently last CDF value. Thereby the histogram columns are being "stacked".
	end

	cdf_values = cdf_values ./ cdf_values[end] # Because the distribution histogram is not exaclty a PDF, this array has to be normalised to 1.
	
	cdf_plot = histogram(bin_positions, weights=cdf_values, bins=62, xlabel="Mass (MeV/c²)", ylabel="Cumulated probability", title="CDF of Resonance Data", legend=false)
	savefig("ResonanceCDF.png")
	cdf_plot
end
...
function hit_and_miss_sampling(bin_contents, bin_positions, num_samples)	
	...
	for i in 1:num_samples
		random_x = trunc(Int, rand() * (lastindex(bin_positions)) + 1)
		random_y = rand() * max_bin_content
		
    	### Position of hit is already set by (random_x,random_y), implement check whether it should be accepted or not
		
		if random_y <= bin_contents[random_x]
			accepted_samples_hm[random_x] = accepted_samples_hm[random_x] .+ 1
			# If the y-value is below the edge of the PDF, 1 will be added to the histogram column at the respective x-value.
		end
	end
...
begin
	...
	plot8 = histogram(bin_positions, weights=bin_contents./maximum(bin_contents), bins=62, xlabel="Mass (MeV/c²)", ylabel="Probability", title="Histogram of Resonance Data", legend=false)
	histogram!(bin_positions, weights=accepted_samples_hm./maximum(accepted_samples_hm), bins=62, xlabel="Mass (MeV/c²)", ylabel="Probability", title="Histogram of Resonance Data", legend=false)
	# For convenience, all plots will be normalised. Thereby showing the PDFs directly.
	savefig("hmPDF.png")
	plot8
end
...
function inverse_cdf_sampling(cdf_values, bin_positions, num_samples)
	...
	for i in 1:num_samples
		...
		u = rand() # Because the CDF is normalised to 1, a standard random number generator for the interval [0,1] can be used.
...
begin
	...
	### Plot histogram with the simulated sample
	plot9 = histogram(bin_positions, weights=bin_contents./maximum(bin_contents), bins=62, xlabel="Mass (MeV/c²)", ylabel="Probability", title="Histogram of Resonance Data", legend=false)
	histogram!(bin_positions, weights=accepted_samples_inv ./ maximum(accepted_samples_inv), bins=62, xlabel="Mass (MeV/c²)", ylabel="Probability", title="Histogram of Resonance Data", legend=false)
	savefig("invPDF.png")
	plot9
end
...
function chi2(sample, data)
    ...
    chi2_values = (data .- sample).^2 # Square of the difference between actual data and simulated data.
    sum(chi2_values)
end

# ╔═╡ b0b544b1-e26c-49d8-87cf-1002142534c7
chi2_hm = chi2(accepted_samples_hm./maximum(accepted_samples_hm), bin_contents./maximum(bin_contents))### Calculate chi2 of Hit and miss method

# Because the munber of data points and the number of simulated data points is not the same, both have been normalised to 1.

# ╔═╡ 7c274fb6-ccfe-484e-9c5b-321225186cb6
chi2_inv = chi2(accepted_samples_inv./maximum(accepted_samples_inv), bin_contents./maximum(bin_contents))### Calculate chi2 of Inverse CDF method

# ╔═╡ d2c1d638-f2c1-4834-9235-a9d55dbaa866
function efficiency(sample, num_samples)
	### Implement calculation of efficiency
	return sum(sample)/num_samples # Efficiency is the ratio of successfully used points to all generated points.
end
...
# ╔═╡ cfe253a5-c57f-4019-8089-168c14c9cda3
...
function momentum_from_circle(point1, point2, q, B)
	...
	p = uconvert(u"GeV/c", p) # Addition: "p = "
...
begin
	...
	for M in M_mumu
		# From the generated J/psi mass (and internally generated sherical coordinates), the momenta of the resulting muons are calculated in cartesian coordinates.
		p1, p2 = jpsi_to_mumu(M)

		# The momenta of the muons are calculated from their momentum components and added to the above lists.
		push!(pmu_minus, sqrt(p1[2]^2 + p1[3]^2 + p1[4]^2))
		push!(pmu_plus, sqrt(p2[2]^2 + p2[3]^2 + p2[4]^2))

		# Here the starting values are collected for the runge_kutta_4 function according to the scheme (t,x,y,z,En,px,py,pz).
		r0_minus = [tmu0, xmu0, ymu0, zmu0, p1[1], p1[2], p1[3], p1[4]]
		r0_plus = [tmu0, xmu0, ymu0, zmu0, p2[1], p2[2], p2[3], p2[4]]

		# While point1 is just r0 for the respective muon, point2 is the simulated point accrding to the eom funtion, which accounts for the em-force.
		point1_minus, point2_minus = runge_kutta_4(eom, r0_minus, par_minus, tmumax, dtmu)
		point1_plus, point2_plus = runge_kutta_4(eom, r0_plus, par_plus, tmumax, dtmu)

		# From there given and simulated points, the trajectory radius and the momentum are calculated.
		R_minus, p_minus = momentum_from_circle(point1_minus, point2_minus, -e, B_exp)
		R_plus, p_plus = momentum_from_circle(point1_plus, point2_plus, e, B_exp)

		# The momenta, calculated from simulated points, are added to the above lists.
		push!(pmu_minus_sim, p_minus)
		push!(pmu_plus_sim, p_plus)

		# To calculate the simulated J/psi mass, the jpsi_from_mumu function is used with the momentum components from the simulated point2_minus and point2_plus. These should in theory be the same as at the moment of decay according to conservation of energy and momentum.
		M_mumu_sim_point = uconvert(u"GeV/c^2", jpsi_from_mumu(point2_minus[5:8], point2_plus[5:8]))
		push!(M_mumu_sim, M_mumu_sim_point)
...
# Comparison
begin
	println("M_mumu = ", mean(M_mumu), " ; ", "M_mumu_sim = ", mean(M_mumu_sim))
